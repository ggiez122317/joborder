    ...
    <script>
      ...
      $(document).on('submit', '#formAuthentication', function(e) {
          ...
          $.ajax({
              ...
              dataType 	: 'JSON', 
              beforeSend 	: function() { 
                  btnLoading('#btnLogin', '<span class="spinner-border spinner-border-sm" role="status" aria-hidden="true"></span><i class="bx bx-lock-open ms-1"></i>Sign in')
              }
          }).done(function(res) {
              if (res.response.status == 1) {
                  window.location.href = '{{ url('/') }}' + res.module
              } else {
                  Swal.fire(
                  res.response.message[0],
                  res.response.message[1],
                  'warning'
                  )
                  btnLoading('#btnLogin', '<i class="bx bx-lock-open"></i>Sign in', 0)
              }
          }).fail(function(err) {
              console.log(err)
          })
