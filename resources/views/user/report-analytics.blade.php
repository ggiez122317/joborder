...
@section('page_title', 'My Report Analytics')
@section('page_subtitle', 'Smart filters, summaries, and breakdowns for your PDS records')
...
@section('content')
    ...
    <div class="mb-5 flex flex-wrap items-start justify-between gap-4">
        <div class="max-w-3xl">
            <div class="text-[11px] font-semibold uppercase tracking-[0.12em] text-[#16a34a]">My Analytics Workspace</div>
            <h1 class="mt-2 text-2xl font-bold uppercase text-[#0f172a]">Report Analytics</h1>
            <p class="mt-2 text-sm leading-6 text-[#64748b]">Use filters to narrow the report, review quick insights, and understand your records at a glance.</p>
        </div>
        <div class="flex flex-wrap gap-3">
            <a href="{{ route('user.records') }}" class="btn-secondary">Back to Records</a>
        </div>
    </div>
    ...
    <section class="panel mb-5 overflow-hidden">
        <div class="border-b border-[#eef2f6] bg-[linear-gradient(135deg,#f8fbf9,white)] px-5 py-4">
            <div class="flex flex-wrap items-center justify-between gap-3">
                <div>
                    ...
                    <div class="mt-1 text-xs text-[#64748b]">Choose the exact records you want to analyze.</div>
                </div>
        ...
        <form method="GET" action="{{ route('user.report-analytics') }}" class="grid gap-4 p-5 md:grid-cols-2 xl:grid-cols-4">
            <label class="block">
                <span class="form-label">Keyword Search</span>
                <input name="q" value="{{ $filters['q'] ?? '' }}" class="form-input mt-2" placeholder="Name, office, position...">
            </label>
            ...
            <div class="flex flex-wrap gap-3 md:col-span-2 xl:col-span-4">
                ...
                <a href="{{ route('user.report-analytics') }}" class="btn-secondary min-w-[120px]">Clear</a>
            </div>
        </form>
    ...
    <section class="mb-5 grid gap-4 lg:grid-cols-2">
        ...
        <div class="grid gap-4 lg:grid-cols-2">
            ...
            <div class="panel p-5">
                <div class="mb-4 text-[11px] font-semibold uppercase tracking-[0.08em] text-[#94a3b8]">Completion Summary</div>
    ...
    <section class="panel overflow-hidden">
        <div class="flex flex-wrap items-center justify-between gap-3 border-b border-[#eef2f6] px-4 py-4">
            <div>
                ...
                <div class="mt-1 text-xs text-[#64748b]">A quick look at the newest filtered records.</div>
            </div>
            <a href="{{ route('user.records') }}" class="btn-secondary">View All Records</a>
        </div>
            ...
            const sexOptions = {
                ...
                plotOptions: {
                    pie: {
                        donut: {
                            size: '50%',
                            labels: { show: true, total: { show: true, label: 'GENDER', formatter: function () { return '' } } }
                        }
                    }
                },
                dataLabels: {
                ...
                legend: { show: true, position: 'bottom', fontSize: '11px', markers: { radius: 12 }, itemMargin: { horizontal: 10, vertical: 5 } }
            };

            new ApexCharts(document.querySelector("#sexApexChart"), sexOptions).render();

            new Chart(document.getElementById('completionPieChart'), {
                type: 'doughnut',
                data: {
                    labels: ['Complete', 'Incomplete'],
                    datasets: [{
                        data: [{{ $totals['complete'] ?? 0 }}, {{ $totals['incomplete'] ?? 0 }}],
