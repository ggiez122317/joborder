...
@section('scripts')
    <script>
        ...
        function getItems(doc, pageWidth)
        {
            ...
            apiCall(`/api/{{ "$controller" }}/print-items-main/?{!! request()->getQueryString() !!}`, 'GET', null, 
                // beforesend
                function() {
                    ...
                    if (res.status == 200) {
                        ...
                        if (res.items.records.length > 0) {
                            for (key in res.items.records) {
                                tblData.push({
                                    "dateInserted" : res.items.records[key].dateInserted, 
                                    "module"    : res.items.records[key].module, 
                                    "action"    : res.items.records[key].action, 
                                    "username"  : res.items.records[key].username, 
                                    "ipAddress" : res.items.records[key].ipAddress, 
                                    "userAgent" : res.items.records[key].userAgent, 
                                    "remarks"   : res.items.records[key].remarks, 
                                })
                            }
                        }
