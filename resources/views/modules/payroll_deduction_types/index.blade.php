            ...
            <div class="table-responsive text-nowrap">
                <table class="table mb-0">
                    <thead>
                        <tr>
                            <th class="text-start">
                                <a href="javascript:void(0);" class="d-flex align-items-center filterSort" data-label="Deduction Code" data-field="code">
                                    <span>Deduction Code</span>
                                </a>
                            </th>
                            <th class="text-start">
                                <a href="javascript:void(0);" class="d-flex align-items-center filterSort" data-label="Name" data-field="name">
                                    <span>Name</span>
                                </a>
                            </th>
                            <th class="text-start">
                                <a href="javascript:void(0);" class="d-flex justify-content-center align-items-center filterSort" data-label="Show In Report" data-field="show">
                        ...
                        <tr valign="middle">
                            ...
                            <th class="text-center">
                                <select name="showInReport" class="form-control">
                                    <option value="">&nbsp;</option>
                                </select>
                            </th>
                            <th class="text-center">
            ...
            apiCall(`/api/{{ "$controller" }}/?${getFilterItems()}`, 'GET', null, 
                ...
                function(res) {
                    ...
                    if (res.status == 200) {
                        ...
                        html += `<option value="1" ${res.items.filters.showInReport === '1'?'selected':''}>Yes</option>`
                        html += `<option value="0" ${res.items.filters.showInReport === '0'?'selected':''}>No</option>`
                        $(`#${formID} select[name="showInReport"]`).html(html)
                            ...
                            for (key in res.items.records) {
                                $(`#${formID} table tbody`).append(`
                                    <tr>
                                        ...
                                        <td class="text-center"><span class="badge bg-${res.items.records[key].showInReport?'success':'danger'}">${res.items.records[key].showInReport?'Yes':'No'}</span></td>
                                        <td class="text-center">
                                            ${res.items.hasButtonView?`
                                                <button type="button" class="btn btn-info btn-sm" onclick="window.location.href='{{ url("/$controller/view") }}/${res.items.records[key].payrollDeductionTypeID}/'">
                                                    <span>
