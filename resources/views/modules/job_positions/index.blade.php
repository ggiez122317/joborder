            ...
            <div class="table-responsive text-nowrap">
                <table class="table mb-0">
                    <thead>
                        <tr>
                            <th class="text-start">
                                <a href="javascript:void(0);" class="d-flex align-items-center filterSort" data-label="Job Position Code" data-field="code">
                                    <span>Job Position Code</span>
                                </a>
                            </th>
                            <th class="text-start">
                                <a href="javascript:void(0);" class="d-flex align-items-center filterSort" data-label="Job Position Name" data-field="name">
                                    <span>Job Position Name</span>
                                </a>
                            </th>
                            <th class="text-start">
                                <a href="javascript:void(0);" class="d-flex align-items-center filterSort" data-label="Office" data-field="office">
                                    <span>Office</span>
                                </a>
                            </th>
                            <th class="text-start">
                                <a href="javascript:void(0);" class="d-flex align-items-center filterSort" data-label="Immediate Superior" data-field="superior">
                                    <span>Immediate Superior</span>
                                </a>
                            </th>
                            <th class="text-start">
                                <a href="javascript:void(0);" class="d-flex justify-content-center align-items-center filterSort" data-label="Mayor" data-field="superior">
                                    <span>Mayor</span>
                                </a>
                            </th>
                            <th class="text-start">
                                <a href="javascript:void(0);" class="d-flex justify-content-center align-items-center filterSort" data-label="HR Head" data-field="superior">
                                    <span>HR Head</span>
                                </a>
                            </th>
                    ...
                    <tbody class="table-border-bottom-0">
                        ...
                        <tr><td class="text-start" colspan="5">Loading...</td></tr>
                    ...
                    if (res.status == 200) {
                        ...
                        if (res.items.offices.length > 0) {
                            ...
                            for (key in res.items.offices) {
                                html += `<option value="${res.items.offices[key].officeID}" ${res.items.offices[key].officeID==res.items.filters.officeID?'selected':''}>${res.items.offices[key].code}${res.items.offices[key].name?` - ${res.items.offices[key].name}`:''}</option>`
                            }
                        }
                        ...
                        if (res.items.JobPositions.length > 0) {
                            ...
                            for (key in res.items.JobPositions) {
                                html += `<option value="${res.items.JobPositions[key].jobPositionID}" ${res.items.JobPositions[key].jobPositionID==res.items.filters.head_positionID?'selected':''}>${res.items.JobPositions[key].code}${res.items.JobPositions[key].name?` - ${res.items.JobPositions[key].name}`:''}</option>`
                            }
                        }
                            ...
                            for (key in res.items.records) {
                                $(`#${formID} table tbody`).append(`
                                    <tr>
                                        ...
                                        <td class="text-start">${res.items.records[key].office}</td>
                                        <td class="text-start">${res.items.records[key].superior}</td>
                                        <td class="text-center"><span class="badge bg-${res.items.records[key].isMayor?'success':'danger'}">${res.items.records[key].isMayor?'YES':'NO'}</span></td>
                                        <td class="text-center"><span class="badge bg-${res.items.records[key].isHrHead?'success':'danger'}">${res.items.records[key].isHrHead?'YES':'NO'}</span></td>
                                        <td class="text-center">
                                            ${res.items.hasButtonView?`
                                                <button type="button" class="btn btn-info btn-sm" onclick="window.location.href='{{ url("/$controller/view") }}/${res.items.records[key].jobPositionID}/'">
                                                    <span>
