                    ...
                    if (res.status == 200) {
                        ...
                        if (res.items.records.length > 0) {
                            for (key in res.items.records) {
                                tblData.push({
                                    "user"              : res.items.records[key].user, 
                                    "username"          : res.items.records[key].username, 
                                    "utName"            : res.items.records[key].utName, 
                                    "dateInserted"      : res.items.records[key].dateInserted, 
                                    "dateActivated"     : res.items.records[key].dateActivated, 
                                    "dateDeactivated"   : res.items.records[key].dateDeactivated, 
                                    "status"            : res.items.statuses[parseInt(res.items.records[key].status)+1], 
                                })
                            }
                        }
