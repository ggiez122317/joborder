                ...
                <table class="table mb-0">
                    <thead>
                        <tr>
                            ...
                            <th class="text-start">
                                <a href="javascript:void(0);" class="d-flex align-items-center filterSort" data-label="Zipcode" data-field="zipcode">
                                    <span>Zipcode</span>
                                </a>
                            </th>
                            <th class="text-start">
                                <a href="javascript:void(0);" class="d-flex align-items-center filterSort" data-label="Province" data-field="pName">
                                    <span>Province</span>
                                </a>
                            </th>
            ...
            apiCall(`/api/{{ "$controller" }}/?${getFilterItems()}`, 'GET', null, 
                ...
                function(res) {
                    ...
                    if (res.status == 200) {
                        ...
                        $(`#${formID} input[name="zipcode"]`).val(`${res.items.filters.zipcode}`)

                        html = ''
                        if (res.items.provinces.length > 0) {
                            html = '<option value="">&nbsp;</option>'
                            for (key in res.items.provinces) {
                                html += `<option value="${res.items.provinces[key].provinceID}" ${res.items.provinces[key].provinceID==res.items.filters.provinceID?'selected':''}>${res.items.provinces[key].name}</option>`
                            }
                        }
                            ...
                            for (key in res.items.records) {
                                $(`#${formID} table tbody`).append(`
                                    <tr>
                                        ...
                                        <td class="text-start">${res.items.records[key].zipcode}</td>
                                        <td class="text-start">${res.items.records[key].pName}</td>
                                        <td class="text-center">
                                            ${res.items.hasButtonView?`
                                                <button type="button" class="btn btn-info btn-sm" onclick="window.location.href='{{ url("/$controller/view") }}/${res.items.records[key].cityID}/'">
                                                    <span>
