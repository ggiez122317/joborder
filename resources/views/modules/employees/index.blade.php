                ...
                <table class="table mb-0">
                    <thead>
                        <tr>
                            ...
                            <th class="text-start">
                                <a href="javascript:void(0);" class="d-flex align-items-center filterSort" data-label="Last Name" data-field="lname">
                                    <span>Last Name</span>
                                </a>
                            </th>
                            <th class="text-start">
                                <a href="javascript:void(0);" class="d-flex align-items-center filterSort" data-label="First Name" data-field="fname">
                                    <span>First Name</span>
                                </a>
                            </th>
                            <th class="text-start">
                                <a href="javascript:void(0);" class="d-flex align-items-center filterSort" data-label="Middle Name" data-field="mname">
                                    <span>Middle Name</span>
                                </a>
                            </th>
                            <th class="text-start">
                                <a href="javascript:void(0);" class="d-flex justify-content-center align-items-center filterSort" data-label="Gender" data-field="gender">
                                    <span>Gender</span>
                                </a>
                            </th>
                            <th class="text-start">
                                <a href="javascript:void(0);" class="d-flex justify-content-center align-items-center filterSort" data-label="Birthday" data-field="birthDate">
                                    <span>Birthday</span>
                                </a>
                            </th>
                        ...
                        <tr valign="middle">
                            ...
                            <th class="text-center">
                                <select name="gender" class="form-control m-auto" style="width: auto;">
                                    <option value="">&nbsp;</option>
                                </select>
                            </th>
            ...
            apiCall(`/api/{{ "$controller" }}/?${getFilterItems()}`, 'GET', null, 
                ...
                function(res) {
                    ...
                    if (res.status == 200) {
                        ...
                        $(`#${formID} input[name="lname"]`).val(`${res.items.filters.lname}`)
                        $(`#${formID} input[name="fname"]`).val(`${res.items.filters.fname}`)
                        $(`#${formID} input[name="mname"]`).val(`${res.items.filters.mname}`)
                        ...
                        if (res.items.genders.length > 0) {
                            ...
                            for (key in res.items.genders) {
                                html += `<option value="${res.items.genders[key].gender}" ${res.items.genders[key].gender==res.items.filters.gender?'selected':''}>${res.items.genders[key].name}</option>`
                            }
                        }
                            ...
                            for (key in res.items.records) {
                                ...
                                $(`#${formID} table tbody`).append(`
                                    <tr>
                                        ...
                                        <td class="text-start">${res.items.records[key].lname}</td>
                                        <td class="text-start">${res.items.records[key].fname}</td>
                                        <td class="text-start">${res.items.records[key].mname}</td>
                                        <td class="text-center">${res.items.records[key].gender}</td>
                                        <td class="text-center">${res.items.records[key].birthDate}</td>
                                        <td class="text-center">
                                            ${res.items.hasButtonView?`
                                                <button type="button" class="btn btn-info btn-sm" onclick="window.location.href='{{ url("/$controller/view") }}/${res.items.records[key].userPersonalInformationID}/'">
                                                    <span>
