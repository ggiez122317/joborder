...
<head>
    <meta charset="UTF-8">
    <title>Valid ID Card - {{ $employee->full_name }}</title>
    <style>
    ...
    <div class="print-grid">
        ...
        <div class="id-container-back {{ \App\Models\IdTemplate::getActiveBackImageUrl() ? 'has-back-template' : '' }}">
            ...
            <div class="back-terms">
                This ID is non-transferable and shall be confiscated when used by others. Please return to the owner if
                found.<br>
                Wear this ID Card when entering the Trento Municipal Hall premises and present to guard on duty upon
                demand.
            </div>
            ...
            <div class="back-details-grid">
                ...
                <div class="back-details-value">:
                    {{ $personal && $personal->philhealth_no ? $personal->philhealth_no : 'N/A' }}</div>
                ...
                <div class="back-details-value">:
                    {{ $personal && $personal->blood_type ? trim(strtoupper($personal->blood_type), '"\'') : 'N/A' }}
                </div>

                <div class="back-details-label">HDMF No.</div>
                <div class="back-details-value">:
                    {{ $personal && $personal->pagibig_id_no ? $personal->pagibig_id_no : 'N/A' }}</div>

                <div class="back-details-label">GSIS No.</div>
                <div class="back-details-value">:
                    {{ $personal && $personal->umid_id_no ? $personal->umid_id_no : 'N/A' }}</div>
            </div>
            ...
            <div class="back-signatory-section" style="position: relative;">
                @if ($employee->e_signature_path)
                    <div style="position: absolute; bottom: 0px; left: 50%; transform: translateX(-50%); z-index: 5;">
                        <img src="{{ route('profile.public.signature', $employee) }}"
                            style="width: 170px; height: 70px; object-fit: contain; mix-blend-mode: multiply; filter: contrast(3) brightness(1.3) grayscale(1);">
                    </div>
                    <div class="back-signatory-name" style="text-transform: uppercase; margin-top: 55px;">
                        {{ $signatoryName }}</div>
                @else
            ...
            <div class="back-template-overlay">
                ...
                <div class="val-philhealth">
                    {{ $personal && $personal->philhealth_no ? $personal->philhealth_no : 'N/A' }}</div>
                <div class="val-birth">{{ $birthDate }}</div>
                <div class="val-blood">
                    {{ $personal && $personal->blood_type ? trim(strtoupper($personal->blood_type), '"\'') : 'N/A' }}
                </div>
                <div class="val-hdmf">{{ $personal && $personal->pagibig_id_no ? $personal->pagibig_id_no : 'N/A' }}
                </div>
