        ...
        <section class="auth-hero relative hidden overflow-hidden lg:block">
            ...
            <div class="relative flex h-full flex-col justify-between p-10 text-white">
                <div>
                    ...
                    <div class="mt-3 text-4xl font-bold leading-tight">Verify your login with a secure code.</div>
                </div>
                <div class="max-w-lg">
                    <p class="text-base leading-7 text-white/80">A 6-digit verification code was sent to your email. The code stays valid for 5 minutes, and you can request a fresh one after 1 minute.</p>
                </div>
            </div>
        </section>
            ...
            <div class="w-full max-w-md">
                <section class="overflow-hidden rounded-[28px] border border-[#dbe5e1] bg-white shadow-[0_30px_70px_rgba(15,23,42,0.10)]">
                    <div class="border-b border-[#edf2ef] bg-[radial-gradient(circle_at_top_left,_rgba(22,163,74,0.12),_transparent_50%),linear-gradient(135deg,#ffffff,#f8fbfa)] px-6 py-6">
                        <div class="text-xs font-semibold uppercase tracking-[0.18em] text-[#16a34a]">Two-Factor Verification</div>
                        <h1 class="mt-3 text-2xl font-bold text-[#0f172a]">Enter your verification code</h1>
                        <p class="mt-2 text-sm leading-6 text-[#64748b]">Check your email inbox for a 6-digit code and enter it below to continue.</p>
                    </div>
                    ...
                    <div class="space-y-5 px-6 py-6">
                        <div class="rounded-[18px] border border-[#e5efe8] bg-[#f8fbf9] px-4 py-4">
                            ...
                            <div class="mt-2 text-sm leading-6 text-[#64748b]">
                                The verification code expires after <span class="font-semibold text-[#0f172a]">5 minutes</span>.
                                If the code expires, tap <span class="font-semibold text-[#0f172a]">Resend Code</span>.
                                This browser will be trusted after successful verification.
                            </div>
                        </div>

                        <form method="POST" action="{{ route('login.two-factor.verify') }}" class="space-y-4">
                            @csrf
                            <div>
                        ...
                        <div class="grid gap-3">
                            <form method="POST" action="{{ route('login.two-factor.resend') }}">
                                @csrf
