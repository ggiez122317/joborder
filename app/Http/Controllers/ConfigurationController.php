...
class ConfigurationController extends MasterController
{
    ...
    public function __construct()
    {
        ...
        $this->logTitle         = 'Configuration';
        $this->table            = 'configurations';
        $this->tablePrimaryKey  = 'configurationID';
        $this->page             = '';
    ...
    public function items(Request $request)
    {
        ...
        if ($request_token['code'] == 200) {
            ...
            if ($this->_checkAccess($token_userID, $this->moduleActionIDs['Index'])) {
                ...
                $query = $query->where('isEditable', 1);
                ...
                if ($temp_records) {
                    foreach ($temp_records as $tr) {
                        $records[] = [
                            'configurationID'   => Crypt::encryptString("{$tr->configurationID}"), 
                            'name'          => $tr->name, 
                            'value'         => $tr->value, 
                            'remarks'       => $tr->remarks, 
                        ];
    ...
    public function get(Request $request, string $id)
    {
        ...
        if ($request_token['code'] == 200) {
            ...
            if ($this->_checkAccess($token_userID, $this->moduleActionIDs['View'])) {
                ...
                /** query */
                if (!$hasError) {
                    $query = Configuration::find($decrypted_id);
                    if ($query) {
                        $row = [
                            'name'      => $query->name, 
                            'value'     => $query->value, 
