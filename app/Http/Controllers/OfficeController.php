...
class OfficeController extends MasterController
{
    ...
    public function __construct()
    {
        ...
        $this->module           = 'Offices';
        $this->controller       = 'offices';
        $this->logTitle         = 'Office';
        $this->table            = 'offices';
        $this->tablePrimaryKey  = 'officeID';
        $this->page             = '';
            ...
            if ($this->_checkAccess($token_userID, $this->moduleActionIDs['Index'])) {
                ...
                if ($temp_records) {
                    foreach ($temp_records as $tr) {
                        $records[] = [
                            'officeID' => Crypt::encryptString("{$tr->officeID}"), 
                            'code'          => $tr->code, 
                            'name'          => $tr->name, 
        ...
        if ($request_token['code'] == 200) {
            ...
            if ($this->_checkAccess($token_userID, $this->moduleActionIDs['Insert'])) {
                ...
                if (!$hasError) {
                    ...
                    if ($hasDuplicate) {
                        $hasError = 1;
                        $data = $this->response->status(409, 'Office code already exist.', 'Invalid!');
                    }
                }
