...
class TaxBracketController extends MasterController
{
    ...
    public function __construct()
    {
        ...
        $this->module           = 'Tax Brackets';
        $this->controller       = 'tax-brackets';
        $this->logTitle         = 'Tax Bracket';
        $this->table            = 'tax_brackets';
        $this->tablePrimaryKey  = 'taxBracketID';
        $this->page             = '';
    ...
    public function items(Request $request)
    {
        ...
        if ($request_token['code'] == 200) {
            ...
            if ($this->_checkAccess($token_userID, $this->moduleActionIDs['Index'])) {
                ...
                /** sort tables */
                $sort_tables = [
                    'salaryTaxableFrom' => $this->table, 
                    'salaryTaxableTo'   => $this->table, 
                    'base'              => $this->table, 
                    'ratePercentage'    => $this->table, 
                    'rateFixed'         => $this->table, 
                ];
                ...
                if ($temp_records) {
                    foreach ($temp_records as $tr) {
                        $records[] = [
                            'taxBracketID'      => Crypt::encryptString("{$tr->taxBracketID}"), 
                            'salaryTaxableFrom' => $tr->salaryTaxableFrom+0?number_format($tr->salaryTaxableFrom, 2):0, 
                            'salaryTaxableTo'   => $tr->salaryTaxableTo+0?number_format($tr->salaryTaxableTo,2):number_format($tr->salaryTaxableFrom,2)."+", 
                            'base'              => number_format($tr->base,2), 
                            'ratePercentage'    => "$tr->ratePercentage%", 
                            'rateFixed'         => number_format($tr->rateFixed,2), 
                        ];
                    }
                }
        ...
        if ($request_token['code'] == 200) {
            ...
            if ($this->_checkAccess($token_userID, $this->moduleActionIDs['View'])) {
                ...
                if (!$hasError) {
                    ...
                    if ($query) {
                        $row = [
                            'salaryTaxableFrom' => $query->salaryTaxableFrom+0?number_format($query->salaryTaxableFrom, 2):0, 
                            'salaryTaxableTo'   => $query->salaryTaxableTo+0?number_format($query->salaryTaxableTo,2):number_format($query->salaryTaxableFrom,2)."+", 
                            'base'              => number_format($query->base,2), 
                            'ratePercentage'    => "$query->ratePercentage%", 
                            'rateFixed'         => number_format($query->rateFixed,2), 
                        ];
        ...
        if ($request_token['code'] == 200) {
            ...
            if ($this->_checkAccess($token_userID, $this->moduleActionIDs['Update'])) {
                ...
                if (!$hasError) {
                    ...
                    if ($query) {
                        $row = [
                            'salaryTaxableFrom' => $query->salaryTaxableFrom, 
                            'salaryTaxableTo'   => $query->salaryTaxableTo, 
                            'base'              => $query->base, 
                            'ratePercentage'    => $query->ratePercentage, 
                            'rateFixed'         => $query->rateFixed, 
                        ];
    ...
    public function put(Request $request, string $id)
    {
        ...
        if ($request_token['code'] == 200) {
            ...
            if ($this->_checkAccess($token_userID, $this->moduleActionIDs['Update'])) {
                /** fields */
                $request_fields     = ['salaryTaxableFrom', 'salaryTaxableTo', 'base', 'ratePercentage', 'rateFixed'];
